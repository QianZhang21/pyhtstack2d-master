...
class GenRunDir:
    def __init__(self, posdir="POSCAR_dir", posname="*", workdir="work", multilevel=None,
        ...
        multilevel: int
            ...
            If the multilevel is None or 2 ( such as work/MoS2), it means that each directory (formula) has only one POSCAR file.
        subset: str
        ...
        genmode: str
            ...
            "qe": The mode to generate the input files for Quantum Espresso calculations.
            "pwdft": The mode to generate the input files for PWDFT calculations.
        takname: str
    ...
    def __runsh_multipos_main(self):
        ...
        with open(self.runfile, "a", newline='\n') as f:
            ...
            f.write("    " * (self.multilevel - 2) + "            if [ -f runlog ] && [ `grep -c \"or copy CONTCAR\" runlog` -ne 0 ]; then\n")
            f.write("    " * (self.multilevel - 2) + "                mv POSCAR POSCAR00\n")
            f.write("    " * (self.multilevel - 2) + "                cp CONTCAR POSCAR\n")
            f.write("    " * (self.multilevel - 2) + "                $MPI $VASP > runlog\n")
    ...
    def failrw(self):
        """
        Write the run_rw.sh file to rework the failed calculation.
        """
...
class GenMultiSh:
    def __init__(self, tasklist=None, workdir="work", multilevel=None,
                 ...
                 opts=True, failact="break", saveall=False, savefiles=None, checkf=True):
        """
    ...
    def part_save(self, sharecommend):
        ...
        with open(self.runfile, "a", newline="\n") as f:
            ...
            if self.opt is not None:
                ...
                f.write(
                    "    " * multi_ + "    if [ ! -s CONTCAR ] || [ `grep -c \"reached required accuracy\" runlog` -eq 0 ]; then\n")
                ...
                f.write("    " * multi_ + "        cp CONTCAR POSCAR\n")
                f.write("    " * multi_ + "        $MPI $VASP > runlog\n")
                f.write("    " * multi_ + "        if [ `grep -c \"reached required accuracy\" runlog` -eq 0 ]; then\n")
                f.write("    " * multi_ + "            cd ..\n")
                f.write("    " * multi_ + f"            echo {file_path}\",\"{self.opt}-failed >> {cd_com}failed.txt\n")
                f.write("    " * multi_ +
                    "            continue\n")
            ...
            f.write("    " * multi_ + "        if [ -f OUTCAR-$task ]; then\n")
            f.write("    " * multi_ + "            continue\n")
            f.write("    " * multi_ + "        fi\n")
    ...
    def full_save(self, sharecommend):
        ...
        with open(self.runfile, "a", newline="\n") as f:
            ...
            if self.opt is not None:
                ...
                f.write("    " * multi_ + f"    if [ ! -s {self.opt}/CONTCAR ] || [ `grep -c \"reached required accuracy\" {self.opt}/runlog` -eq 0 ]; then\n")
            ...
            f.write("    " * multi_ + "        if [ -s $task/OUTCAR ] && [ `grep -c $jobconv $task/OUTCAR` -ne 0 ] && [ `grep -c $jobfinish $task/OUTCAR` -ne 0 ]; then\n")
            f.write("    " * multi_ + "            continue  # The calculation has already been completed.\n")
            f.write("    " * multi_ + "        fi\n")
            ...
            if len(self.bandtask) != 0:
                for band_ in self.bandtask:
                    ...
                    f.write("    " * multi_ + f"    if [ ! -f scf/CHG ] || [ ! -f scf/CHGCAR ]; then\n")
                    f.write("    " * multi_ + "        cd ..\n")
                    f.write("    " * multi_ + f"        continue  # The charge density files are not present, so the band structure calculation cannot be performed.\n")
                    f.write("    " * multi_ + "    fi\n")

                    f.write("    " * multi_ + f"    if [ ! -s {band_}/OUTCAR ] || [ `grep -c $jobconv {band_}/OUTCAR` -eq 0 ] || [ `grep -c $jobfinish {band_}/OUTCAR` -eq 0 ]; then\n")
...
class GenMultiQESh(GenMultiSh):
    """
    This class is used to generate a bash script to run the multiple tasks by Quantum ESPRESSO package.
    """
    ...
    def genRunsh(self, qe="pw.x", moduleload="qe/7.0"):
...
class MulTaskRW(GenMultiSh):
    """
    This class is used to generate a bash script to rework the failed multiple tasks, which only supports VASP calculation.
    For Quantum ESPRESSO, please modify the script manually, which generated by the GenMultiQESh class.
    Note: The failed.txt file is generated by the multi-task calculation script.
    """

    def read_failtask(self):
        if os.path.exists("failed.txt"):
            self.faildir_dict = OrderedDict()
            with open("failed.txt", 'r') as ff:
                content = ff.read()

            assert '#################' in content, "Please check the content of the failed.txt file."

            parts = content.strip().split('#################')
            last_section = parts[-1].strip()

            priority = {'relax': 0, 'opt': 0, 'scf': 1, 'band': 2, 'hybband': 2}

            lines = last_section.split('\n')
            self.multilevel = len((lines[0].split(","))[0].split("/")) + 1

            for line in lines:
                task_split = line.split(",")
                if len(task_split) >= 2:
                    task_name = task_split[1].split('-')[0]
                    if task_name not in self.faildir_dict:
                        temp_list = list(self.faildir_dict.items())
                        index = 0
                        while index < len(temp_list) and priority.get(temp_list[index][0], 3) <= priority.get(task_name,
                                                                                                              3):
                            index += 1
                        temp_list.insert(index, (task_name, []))
                        self.faildir_dict.clear()
                        self.faildir_dict.update(temp_list)
                    self.faildir_dict[task_name].append(task_split[0])
            return self.faildir_dict.keys()
        else:
            assert False, "The failed.txt file does not exist."
    ...
    def part_save(self, sharecommend):
        ...
        if self.tasklist0 is not None:
            self.tasklist0.remove(self.opt)
            failwrt = " ".join(self.tasklist0)
        else:
            failwrt = self.wrt
        ...
        with open(self.runfile, "a", newline="\n") as f:
            f.write("\n")
            if self.opt is not None:
                wft_ = " ".join(self.faildir_dict[self.opt])
                f.write(f"{self.opt}_mid=({wft_})\n")
            for task_ in self.runtask:
                wft_ = " ".join(self.faildir_dict[task_])
                f.write(f"{task_}_mid=({wft_})\n")
            f.write(f"\ntasklist=({failwrt})  # Warning: The order of calculations must be reasonable, e.g. scf must come before band calculations.\n")
            ...
            if self.opt is not None:
                optsharecom = [re.sub(r'\$task', self.opt, cmd) for cmd in sharecommend]
                f.write("# Optimize Structure\n")
                f.write('for mid in ${' + f'{self.opt}' + '_mid[@]}\n')
                f.write("do\n")
                f.write("    if [ ! -d $mid ]; then\n")
                f.write("        echo $mid\" not found\"\n")
                f.write("        continue\n")
                f.write("    fi\n")
                f.write("    cd $mid\n")
                # if self.potshare:
                #     f.write("        ln -nfs ../POTCAR POTCAR\n")
                f.write("    if [ -s CONTCAR ]; then\n")
                f.write("        mv POSCAR POSCAR00\n")
                f.write("        mv CONTCAR POSCAR\n")
                f.write("    fi\n")
                f.write(f"    {optsharecom[0]}\n")
                f.write(f"    {optsharecom[1]}\n")
                f.write("    $MPI $VASP > runlog\n")
                f.write(
                        "    if [ -f runlog ] && [ `grep -c \"or copy CONTCAR\" runlog` -ne 0 ]; then\n")
                f.write("        mv POSCAR POSCAR00\n")
                ...
                f.write("        $MPI $VASP > runlog\n")
                f.write("    fi\n")
                f.write("    if [ ! -f runlog ] || [ `grep -c \"reached required accuracy\" runlog` -eq 0 ]; then\n")
                f.write(f"        echo $mid\",\"{self.opt}-failed >> {cd_com}../failed.txt\n")
                f.write("    fi\n")
                f.write("    mv POSCAR POSCAR0\n")
                f.write("    cp CONTCAR POSCAR\n")
                for savefile in self.savefiles:
                    f.write(f"    mv {savefile} {savefile}-{self.opt}\n")

                if self.opts:
                    f.write("# Optimize Structure Again\n")
                    f.write("    $MPI $VASP > runlog\n")
                    f.write("    if [ ! -f runlog ] || [ `grep -c \"reached required accuracy\" runlog` -eq 0 ]; then\n")
                    f.write(f"        echo $mid\",\"{self.opt}-s-failed >> ../failed.txt\n")
                    f.write("    fi\n")
                    f.write("    mv POSCAR POSCAR1\n")
                    f.write("    cp CONTCAR POSCAR\n")
                    for savefile in self.savefiles:
                        f.write(f"    mv {savefile}-{self.opt} {savefile}-{self.opt}0\n")
                        f.write(f"    mv {savefile} {savefile}-{self.opt}\n")

                f.write(f"# {failwrt}\n")
                f.write("    for task in ${tasklist[@]}\n")
                f.write("    do\n")
                f.write(f"        {sharecommend[0]}\n")
                f.write(f"        {sharecommend[1]}\n")
                f.write("        $MPI $VASP > runlog\n")
                for savefile in self.savefiles:
                    f.write(f"        cp {savefile} {savefile}-$task\n")
                f.write(
                        "        if [ ! -f OUTCAR ] || [ `grep -c $jobconv OUTCAR` -eq 0 ] || [ `grep -c $jobfinish OUTCAR` -eq 0 ]; then\n")
                f.write( f"            echo $mid\",\"$task-failed >> {cd_com}../failed.txt\n")
                f.write(f"            {self.failact}\n")
                f.write("        fi\n")
                f.write( "    done\n")
            ...
            for task_ in self.runtask:
                tasksharecom = [re.sub(r'\$task', task_, cmd) for cmd in sharecommend]
                f.write("\nfor mid in ${" + f"{task_}_mid[@]" + "}\n")
                f.write("do\n")
                f.write("    if [ ! -d $mid ]; then\n")
                f.write("        echo $mid\" not found\"\n")
                f.write("        continue\n")
                ...
                # if self.potshare:
                #     f.write("        ln -nfs ../POTCAR POTCAR\n")
                f.write(f"    {tasksharecom[0]}\n")
                f.write(f"    {tasksharecom[1]}\n")
                f.write("    $MPI $VASP > runlog\n")
                for savefile in self.savefiles:
                    f.write(f"    cp {savefile} {savefile}-{task_}\n")
                f.write(
                    "    if [ ! -f OUTCAR ] || [ `grep -c $jobconv OUTCAR` -eq 0 ] || [ `grep -c $jobfinish OUTCAR` -eq 0 ]; then\n")
                f.write(f"        echo $mid\",\"{task_}-failed >> {cd_com}../failed.txt\n")
                f.write("    fi\n")
                f.write(f"    cd {cd_com}..\n")
                f.write("done\n")
    ...
    def full_save(self, sharecommend):
        ...
        if self.tasklist0 is not None:
            self.tasklist0.remove(self.opt)
            failbandtask = []
            if "band" in self.tasklist0:
                self.tasklist0.remove("band")
                failbandtask.append("band")
            if "hybband" in self.tasklist0:
                self.tasklist0.remove("hybband")
                failbandtask.append("hybband")
            failwrt = " ".join(self.tasklist0)
        else:
            failwrt = " ".join([x for x in self.runtask if x not in self.bandtask])
            failbandtask = self.bandtask
        ...
        with open(self.runfile, "a", newline="\n") as f:
            ...
            if self.opt is not None:
                ...
                # if self.potshare:
                #     f.write("        ln -nfs ../POTCAR POTCAR\n")
                f.write(f"    cd {self.opt}\n")
                f.write(f"    cp ../POSCAR POSCAR\n")
                f.write(f"    ln -nfs ../POTCAR POTCAR\n")
                f.write(f"    {optsharecom[0]}\n")
                f.write(f"    {optsharecom[1]}\n")
                f.write("    $MPI $VASP > runlog\n")
                f.write(
                ...
                if self.opts:
                    ...
                    f.write("    $MPI $VASP > runlog\n")
                    f.write("    if [ ! -f runlog ] || [ `grep -c \"reached required accuracy\" runlog` -eq 0 ]; then\n")
                    f.write(f"        echo $mid\",\"{self.opt}-s-failed >> {cd_com}../../failed.txt\n")
                    f.write("    fi\n")
                    f.write("    mv POSCAR POSCAR1\n")
                    f.write("    cp CONTCAR ../POSCAR\n")
                f.write("    cd ..\n")
                ...
                f.write("    for task in ${tasklist[@]}\n")
                f.write("    do\n")
                f.write("        if [ ! -d $task ]; then\n")
                f.write("            mkdir $task\n")
                f.write("        fi\n")
                f.write("        cd $task\n")
                f.write("        ln -nfs ../POSCAR POSCAR\n")
                f.write("        ln -nfs ../POTCAR POTCAR\n")
                f.write(f"        {sharecommend[0]}\n")
                f.write(f"        {sharecommend[1]}\n")
                f.write("        $MPI $VASP > runlog\n")
                ...
                if len(failbandtask) != 0:
                    for band_ in failbandtask:
                        bandsharecom = [re.sub(r'\$task', band_, cmd) for cmd in sharecommend]
                        f.write(f"    if [ ! -d {band_} ]; then\n")
                        f.write(f"        mkdir {band_}\n")
                        f.write("    fi\n")
                        f.write(f"    cd {band_}\n")
                        f.write("    ln -nfs ../POSCAR POSCAR\n")
                        f.write("    ln -nfs ../POTCAR POTCAR\n")
                        f.write(f"    {bandsharecom[0]}\n")
                        f.write(f"    {bandsharecom[1]}\n")
                        if "band" in band_:
                            f.write("    ln -nfs ../scf/CHG CHG\n")
                            f.write("    ln -nfs ../scf/CHGCAR CHGCAR\n")
                        if "hyb" in band_:
                            f.write("    ln -nfs ../scf/WAVECAR WAVECAR\n")
                        f.write( "    $MPI $VASP > runlog\n")
                        f.write(
                                "    if [ ! -f OUTCAR ] || [ `grep -c $jobconv OUTCAR` -eq 0 ] || [ `grep -c $jobfinish OUTCAR` -eq 0 ]; then\n")
                        f.write(
                            f"        echo $mid\",\"{band_}-failed >> {cd_com}../../failed.txt\n")
                        f.write("    fi\n")
                        f.write("    cd ..\n")
                f.write(f"    cd {cd_com}..\n")
                f.write("done\n")

                tasknb = [x for x in self.runtask if x not in self.bandtask]
                for task_ in tasknb:
                    tasksharecom = [re.sub(r'\$task', task_, cmd) for cmd in sharecommend]
                    f.write("\nfor mid in ${" + f"{task_}_mid[@]" + "}\n")
                    f.write("do\n")
            ...
            if len(self.bandtask) > 0:
                for band_ in self.bandtask:
                    ...
                    f.write(
                        "    if [ ! -f OUTCAR ] || [ `grep -c $jobconv OUTCAR` -eq 0 ] || [ `grep -c $jobfinish OUTCAR` -eq 0 ]; then\n")
                    f.write(f"        echo $mid\",\"{band_}-failed >> {cd_com}../../failed.txt\n")
                    f.write("    fi\n")
                    f.write(f"    cd {cd_com}../..\n")
                    f.write("done\n")
...
class OptPOSCAR:
    def __init__(self, workdir=None, posdir="opt_POSCAR_dir", optpath=None, multilevel=None):
        """
        Get the POSCAR files after the optimization, and save them in "posdir" by the script "get_poscar.sh".

        workdir: str
            The directory of the VASP calculation.
        posdir: str
            The directory to save the optimized POSCAR files.
        optpath: str
            The path of the optimization calculation.
            For example, if the optimization calculation is in the "opt" directory, the optpath is "opt".
            If None, it means that the optimization calculation is in the material directory.
        multilevel: int
            The number of sub-directories that are used to write the run.sh file for several POSCAR files (stored in a multi-level directory) of the same chemical formula.
            The "posname" and "workdir" parameters are invalid if multilevel is not None.
            For example, if the POSCAR files are stored in the directory "BiPOSCAR_dir/S24P8Ag4V4_0.00%/AA/cord1", the multilevel is 4.
        ...
        if self.optpath is None:
            self.optpath = ""
        elif "/" not in self.optpath:
            self.optpath += "/"
        self.getsh = os.path.join(self.workdir, "getPOSCAR.sh")
    ...
    def genGetsh(self):
        ...
        with open(self.getsh, "w", newline="\n") as f:
            f.write("#!/bin/bash\n\n")
            f.write(f"if [ -d ../{self.posdir} ]; then\n")
            f.write(f"    rm -r ../{self.posdir}\n")
            f.write("fi\n")
            f.write(f"mkdir ../{self.posdir}\n\n")
            f.write("for file in `ls $1`\n")
            f.write("do\n")
            f.write("    if [ ! -d $file ]; then\n")
            f.write("        continue\n")
            f.write("    fi\n")
            ...
            f.write("    " * multi_ +
                f"    if [ ! -f {self.optpath}CONTCAR ] || [ ! -f {self.optpath}runlog ] || [ `grep -c \"reached required accuracy\" {self.optpath}runlog` -eq 0 ]; then\n")
            f.write("    " * multi_ + f"        cd ..\n")
            f.write("    " * multi_ + f"        continue\n")
            f.write("    " * multi_ + "    fi\n")
            f.write("    " * multi_ + f"    cp {self.optpath}CONTCAR {cd_com}../../" + self.posdir + f"/{file_path}-POSCAR\n")
...
class GetMagEntropy:
    def __init__(self, workdir=None, magpath=None, multilevel=None):
        """
        Get the magnetic moments and entropy after VASP calculation.

        workdir: str
            The directory of the VASP calculation.
        magpath: str
            The path of the VASP calculation to get the magnetic moments.
        ...
        self.saveall = None
        if magpath is None:
            self.magpath = "OSZICAR"
            self.entropypath = "OUTCAR"
        elif "/" in magpath:
            self.saveall = magpath
            self.magpath = magpath + "OSZICAR"
            self.entropypath = magpath + "OUTCAR"
        elif "-" in magpath:
            self.magpath = f"OSZICAR{magpath}"
            self.entropypath = f"OUTCAR{magpath}"
        else:
            raise ValueError("magpath should be None, or a string with '/' or '-', such as 'scf/' or '-scf'.")
        self.getsh = os.path.join(self.workdir, "getMagEntropy.sh")
    ...
    def genGetsh(self):
        ...
        with open(self.getsh, "w", newline="\n") as f:
            f.write("#!/bin/bash\n\n")
            f.write(f"if [ -f mag.txt ]; then\n")
            f.write(f"    rm -r mag.txt\n")
            f.write("fi\n")
            ...
            f.write("    " * multi_ +
                f"    if [ ! -f {self.magpath} ]; then\n")
            f.write("    " * multi_ + f"        cd ..\n")
            f.write("    " * multi_ + f"        continue\n")
            f.write("    " * multi_ + "    fi\n")
            f.write(
                "    " * multi_ + "    mag=`tail -1 "+f"{self.magpath}"+" | awk \'{print $10}\'` \n")
            f.write("    " * multi_ + f"    echo " + f"{file_path},$mag >> {cd_com}../mag.txt\n")
            f.write(
                "    " * multi_ + "    entropy=`grep \"entropy T\" " + f"{self.entropypath}" +
                " | tail -1 | awk '{print $5}'` \n")
            f.write("    " * multi_ + f"    echo " + f"{file_path},$entropy >> {cd_com}../entropy.txt\n")
    ...
    def separateMag(self, magtxt="mag.txt", magtol=0.1):
        """
        Separate the magnetic system and non-magnetic system.

        magtxt: str
            The file storing the materials id and magnetic moments.
        maglowerl: float
            The theshold to determine whether the material is magnetic or not.
        """
        magtxt = os.path.join(self.workdir, magtxt)
        if not os.path.exists(magtxt):
            import subprocess
            if not os.path.exists(self.getsh):
                self.genGetsh()
            subprocess.call(['bash', self.getsh])

        with open(magtxt, "r") as f:
            for line in f:
                line_info = line.split(",")
                material_id = line_info[0]
                mag = float(line_info[1])
                if abs(mag) > magtol:
                    self.mag_list.append(material_id)
                else:
                    self.nomag_list.append(material_id)

        with open(os.path.join(self.workdir, "maglist.txt"), "w", newline="\n") as f:
            for material_id in self.mag_list:
                f.write(material_id + "\n")

        with open(os.path.join(self.workdir, "nomaglist.txt"), "w", newline="\n") as f:
            for material_id in self.nomag_list:
                f.write(material_id + "\n")

    def separateEntropy(self, entxt="entropy.txt", entropytol=0.001, recalheader=None):
        """
        Systems with an entropy higher than the tolerance value have been separated and recalculated.
        It is necessary to ensure that the 'entropy T*S' after the calculation is less than 1 meV per atom.

        entxt: str
            The file storing the materials id and entropy.
        entropytol: float
            Default is 0.001 eV.
        """
        entxt = os.path.join(self.workdir, entxt)
        if not os.path.exists(entxt):
            import subprocess
            if not os.path.exists(self.getsh):
                self.genGetsh()
            subprocess.call(['bash', self.getsh])
        ...
        entropy_list = []
        with open(entxt, "r") as f:
            for line in f:
                line_info = line.split(",")
                material_id = line_info[0]
                entropy = float(line_info[1])
                if abs(entropy) > entropytol:
                    entropy_list.append(material_id)

        with open(os.path.join(self.workdir, "entropylist.txt"), "w", newline="\n") as f:
            for material_id in entropy_list:
                f.write(material_id + "\n")
        ...
        if recalheader:
            if self.multilevel:
                ...
                cd_com = ""
            os.path.exists(recalheader)
            with open(recalheader, "r") as f:
                recalheader = f.read()
            with open(os.path.join(self.workdir, "runentropy.sh"), "w", newline="\n") as f:
                f.write(recalheader)
            with open(os.path.join(self.workdir, "runentropy.sh"), "a", newline="\n") as f:
                f.write("\n")
                f.write("for file in `cat entropylist.txt`\n")
                f.write("do\n")
                f.write("    cd $file\n")
                if self.saveall:
                    f.write(f"    cd {self.saveall}\n")
                    cd_com += "../"
                f.write("    $MPI $VASP > runlog\n")
                f.write(f"    cd {cd_com}..\n")
                f.write("done\n")
